<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0078FF,100:3776AB&height=180&section=header&text=coding-test&fontSize=42&fontColor=ffffff&animation=twinkling"/>

**프로그래머스 코딩테스트 입문 · 기초 트레이닝 풀이 노트**

풀이 + 오답노트를 md로 정리 (Velog 백업용)

<br>

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Programmers](https://img.shields.io/badge/Programmers-0078FF?style=flat-square)
![Markdown](https://img.shields.io/badge/Markdown-000000?style=flat-square&logo=markdown&logoColor=white)

<br>

![문제](https://img.shields.io/badge/풀이_완료-32문제-blue?style=for-the-badge)
![Lv0](https://img.shields.io/badge/Lv.0-27-green?style=for-the-badge)
![Lv2](https://img.shields.io/badge/Lv.2-2-orange?style=for-the-badge)

<br>

[![Velog](https://img.shields.io/badge/Velog-@choihwanwoo-20c997?style=for-the-badge&logo=velog&logoColor=white)](https://velog.io/@choihwanwoo)
[![GitHub](https://img.shields.io/badge/GitHub-choihwanwoo-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/choihwanwoo)

</div>

---

## About

| | |
|---|---|
| **목적** | 코딩테스트 입문 문제를 풀고, 헷갈린 점을 오답노트로 남기기 |
| **플랫폼** | [프로그래머스](https://programmers.co.kr/) |
| **언어** | Python · Java |
| **노트 형식** | `## 풀이` → `## 오답노트` → `## 정답` |

<br>

### 폴더 구조

```
coding-test/
├── 📁 lv0/       입문 (Lv.0) 27문제
├── 📁 lv2/       Lv.2 2문제
├── 📁 etc/       PCCE · 기타 3문제
├── 📁 assets/    아이콘
└── 📄 README.md
```

---

## 목차

| 레벨 | 문제 수 | 바로가기 |
|------|---------|----------|
| Lv.0 · 코딩테스트 입문 | 27 | [펼치기](#lv0) |
| Lv.2 | 2 | [펼치기](#lv2) |
| 기타 · PCCE | 3 | [펼치기](#etc) |

---

<a id="lv0"></a>

<details open>
<summary><b>📗 Lv.0 · 코딩테스트 입문 (27)</b></summary>
<br>

| # | 문제 | 언어 | 노트 |
|:-:|------|:----:|:----:|
| 01 | 두 수의 나눗셈 | `Java` | [📄](./lv0/두-수의-나눗셈.md) |
| 02 | 두 수의 차 | `Java` | [📄](./lv0/두-수의-차.md) |
| 03 | 문자 출력 | `Java` | [📄](./lv0/문자-출력.md) |
| 04 | 각도 합치기 | `Java` | [📄](./lv0/각도-합치기.md) |
| 05 | 분수의 덧셈 | `Java` | [📄](./lv0/분수의-덧셈.md) |
| 06 | 배열 두배 만들기 | `Java` | [📄](./lv0/배열-두배-만들기.md) |
| 07 | 중앙값 구하기 | `Java` | [📄](./lv0/중앙값-구하기.md) |
| 08 | 최빈값 구하기 | `Java` | [📄](./lv0/최빈값-구하기.md) |
| 09 | 짝수는 싫어요 | `Java` | [📄](./lv0/짝수는-싫어요.md) |
| 10 | 피자 나눠 먹기 (1) | `Java` | [📄](./lv0/피자-나눠-먹기-1.md) |
| 11 | 피자 나눠 먹기 (2) | `Java` | [📄](./lv0/피자-나눠-먹기-2.md) |
| 12 | 배열의 평균값 | `Java` | [📄](./lv0/배열의-평균값.md) |
| 13 | 옷가게 할인 받기 | `Java` | [📄](./lv0/옷가게-할인-받기.md) |
| 14 | 문자 리스트를 문자열로 변환하기 | `Python` | [📄](./lv0/문자-리스트를-문자열로-변환하기.md) |
| 15 | 대소문자 바꿔서 출력하기 | `Python` | [📄](./lv0/대소문자-바꿔서-출력하기.md) |
| 16 | 홀짝 구분하기 | `Python` | [📄](./lv0/홀짝-구분하기.md) |
| 17 | 연속된 수의 합 | `Python` | [📄](./lv0/연속된-수의-합.md) |
| 18 | OX퀴즈 | `Python` | [📄](./lv0/OX퀴즈.md) |
| 19 | 다음에 올 숫자 | `Python` | [📄](./lv0/다음에-올-숫자.md) |
| 20 | 옹알이 (1) | `Java` | [📄](./lv0/옹알이-1.md) |
| 21 | 정수를 나선형으로 배치하기 | `Python` | [📄](./lv0/정수를-나선형으로-배치하기.md) |
| 22 | 겹치는 선분의 길이 | `Python` | [📄](./lv0/겹치는-선분의-길이.md) |
| 23 | 평행 | `Python` | [📄](./lv0/평행.md) |
| 24 | 안전지대 | `Python` | [📄](./lv0/안전지대.md) |
| 25 | 다항식 더하기 | `Python` | [📄](./lv0/다항식-더하기.md) |
| 26 | 코드 처리하기 | `Python` | [📄](./lv0/코드-처리하기.md) |
| 27 | 배열 조각하기 | `Python` | [📄](./lv0/배열-조각하기.md) |

</details>

---

<a id="lv2"></a>

<details>
<summary><b>📙 Lv.2 (2)</b></summary>
<br>

| # | 문제 | 언어 | 노트 |
|:-:|------|:----:|:----:|
| 01 | 최고의 집합 | `Java` | [📄](./lv2/최고의-집합.md) |
| 02 | 점 찍기 | `Java` | [📄](./lv2/점-찍기.md) |

</details>

---

<a id="etc"></a>

<details>
<summary><b>📘 기타 · PCCE / 연습 (3)</b></summary>
<br>

| # | 문제 | 노트 |
|:-:|------|:----:|
| 01 | 수 나누기 (PCCE) | [📄](./etc/수-나누기.md) |
| 02 | 병과 분류 | [📄](./etc/병과분류.md) |
| 03 | 심폐소생술 | [📄](./etc/심폐소생술.md) |

</details>

---

<div align="center">

<br>

### Tech Stack

<img src="./assets/python.png" width="56" alt="Python" />
&nbsp;&nbsp;
<img src="./assets/java.png" width="56" alt="Java" />

<br><br>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0078FF,100:3776AB&height=100&section=footer"/>

</div>
